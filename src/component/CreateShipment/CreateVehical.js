...
async function addBatch(name,email,vehicalplate,phone,setModalIsOpen,getBatchList){
  if (name != "" && email != "" && vehicalplate!= "" && phone!= "" ) {
    await axios.post('http://localhost:5000/vehical/addvehical',
    {
...
<Modal isOpen={modalIsOpen} className='main_modal_body'>
                ...
                <AiOutlineClose className='main_AiOutlineClose' onClick={()=>setModalIsOpen(false)}/>
                   <h5 className='main_h5'>Create Vehical</h5>
                </ModalBody>
                <Form className='form_main'>
                    ...
                    <Button variant="contained" className='main_botton' style={{backgroundColor: '#198754'}} onClick={() => addBatch(name,email,vehicalplate,phone,setModalIsOpen,getBatchList)}>Create Vehical</Button>

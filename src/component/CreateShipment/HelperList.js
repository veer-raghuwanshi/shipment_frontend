...
async function ContactData(getContact){
  ...
  await axios.get('http://localhost:5000/helper/createhelper',
...
async function updateBatch(id,name,email,phone,address,setModalIsOpenEdit,getBatchList){
  if (name != "" && email != "" && phone != "" && address != "") {
      await axios.post('http://localhost:5000/helper/updatehelper',
...
async function deleteContact(ids,getContact,DefaultgetContact ){
  const results = await axios.post('http://localhost:5000/helper/delhelper',
      {
          id:ids
      },
      {headers: { authorization:`Bearer ${localStorage.getItem('token')}` }}

...
async function ContactData(getContact){
  ...
  await axios.get('http://localhost:5000/driver/driver',
...
async function updateBatch(id,full_name,email,phone,address,setModalIsOpenEdit,getBatchList){
  if (full_name != "" && email != "" && phone != "" && address != "") {
      await axios.post('http://localhost:5000/driver/updatedriver',
...
async function deleteContact(ids,getContact,DefaultgetContact ){
  const results = await axios.post('http://localhost:5000/driver/deldriver',
      {
          id:ids
      },
      {headers: { authorization:`Bearer ${localStorage.getItem('token')}` }}
